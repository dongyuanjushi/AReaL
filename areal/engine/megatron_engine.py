...
class MegatronEngine(TrainEngine):
    ...
    def train_batch(
        ...
        def forward_step(batch_iter, model):
            ...
            if self.enable_tree_training:
                ...
                output = model_with_tree_attention_forward(model, batch, dtype=self.dtype, remove_padding=False)
    ...
    def forward(
        ...
        def forward_step(batch_iter, model):
            ...
            if self.enable_tree_training:
                orig_input = mb_list.mbs[forward_step_count]
                output = model_with_tree_attention_forward(model, batch, dtype=self.dtype, remove_padding=True)
                forward_step_counts[model_vp_stage] += 1
            else:
                padding_length = mb_list.padding_lengths[forward_step_count]
                orig_input = mb_list.mbs[forward_step_count]
