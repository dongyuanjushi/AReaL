...
def _visualize_attention_mask(mask_tensor: torch.Tensor, granularity: int = 128) -> None:
...
def packed_tree_gather_logprobs(
    ...
    for seq_len, seq_id in zip(seq_lens, sequence_ids):
        ...
        if calculate_entropy:
            ...
            seq_logprobs = gather_logprobs_from_segments(tree_logits_segments, labels, temperature)
            flattened_logprobs[cursor : cursor + seq_len] = seq_logprobs
        cursor += seq_len
...
if TREE_ATTENTION_BACKEND_TYPE == "pytorch_flex":
    ...
    logger.info("Compiled torch flex attention. Options: %s, dynamic: %s", str(torch_compile_options), str(dynamic))
...
class PytorchScaledDotProductAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if TREE_ATTENTION_BACKEND_TYPE == "pytorch_xformer":
            # _attention_mask = attention_mask.bool().squeeze(0)
            # attention_mask = torch.zeros_like(_attention_mask, dtype=query.dtype)
